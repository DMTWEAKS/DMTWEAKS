...
function ProductCard({ product, priority = false }: ProductCardProps) {
  ...
  return (
    <>
      <Card 
        className="group flex h-full flex-col overflow-hidden border-2 border-primary/20 bg-gradient-to-br from-primary/5 via-background to-background transition-all duration-300 hover:border-primary/50 hover:shadow-xl hover:shadow-primary/10 hover:scale-[1.02]"
      >
            ...
            {product.tags && product.tags.length > 0 && (
              <div className="flex flex-wrap gap-1.5 pt-1">
                {product.tags.slice(0, 2).map((tag, index) => {
                  ...
                  return (
                    <Badge key={tagKey} variant="secondary" className="text-xs font-medium bg-muted/60 border border-border/50">
                      {tagName}
                    </Badge>
                  );
                })}
