...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/v1/store/navlinks`, {
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(error.message || 'Failed to fetch navlinks');
    }
    ...
    return NextResponse.json(
      { success: false, error: error.message || 'Failed to fetch navlinks' },
      { status: 500 }
    );
  }
}
